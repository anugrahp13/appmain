<!-- =============================================== -->
  ...
  <section class="content-header">
    <h1>
      Tambah Kekurangan
      <small>Tambah biaya kekurangan</small>
    </h1>
    <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dollar"></i> Tambahan</a></li>
      <li class="active">Tambah Kekurangan</li>
    </ol>
  </section>
  ...
  <section class="content-header">
    <div class="callout callout-info">
      ...
      Priode tambah kekurangan di sesuaikan dengan periode cetak honor dosen.<br>
      <label>Informasi!</label>
      Banyak tambah kekurangan di isi dengan nominal & Nominal Satuan di isi dengan angka.
    </div>
  </section>
  <!-- Finish Content Header -->
  ...
  <section class="content">
    <?php if($this->session->flashdata('messageactive') == "tkekurangan"){?>
    ...
    <!-- Default Box -->
    <div class="box">
      <form action="<?php echo base_url();?>d.php/tkekurangan/hapus" method="post" id="myform">
        <!-- Box Body -->
        <div class="box-body">
          <!-- Table Bagian Atas -->
          <table id="example1" class="table table-bordered">
            <thead>
              <tr>
                ...
                <th class="text-center">Nominal Satuan</th>
                <th>Priode</th>
                <?php if($this->session->userdata('id_divisi') == "1"){ ?>
            ...
            <tbody>
              <?php
                $no = 0;
                foreach ($loadkekurangan as $key) {
                  $no++;
                  $a = $this->global_model->find_by('dosen_profil', array('id_dosen'=> $key['id_dosen']));
                  if($a != null){
              ?>
              <tr class="record">
                <td id="kode" style="display:none;"><?php echo $key['id_kekurangan'];?></td>
                <td><input type="checkbox" name="check[]" value="<?php echo $key['id_kekurangan'];?>"></td>
                <td><?php echo $no;?></td>
                <td>
                  <?php
                    $a = $this->global_model->find_by('dosen_profil', array('id_dosen'=>$key['id_dosen']));
                ...
                <td class="text-center"><?php echo $key['banyak']; ?></td>
                <td class="text-center"><?php echo $key['nominal_satuan']; ?></td>
                <td>
                  <?php
          ...
          <div class="modal-body">
            <div class="row">
              <div class="col-md-6">
                ...
                <div class="form-group">
                  <label>Banyak</label>
                  <input type="number" name="banyak" class="form-control" onkeypress="return isNumberKey(event)" required>
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-group">
                  <label>Nominal Satuan</label>
                  <input type="number" name="nominal_satuan" class="form-control" onkeypress="return isNumberKey(event)" required>
                </div>
                <div class="form-group">
                  <label>Dari</label>
          ...
          <div class="modal-body">
            <div class="row">
              <div class="col-md-6">
                ...
                <div class="form-group">
                  <label>Banyak</label>
                  <input type="number" name="banyak" class="form-control" onkeypress="return isNumberKey(event)" required id="banyak">
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-group">
                  <label>Nominal</label>
                  <input type="number" name="nominal_satuan" class="form-control" onkeypress="return isNumberKey(event)" required id="bnominal">
                </div>
                <div class="form-group">
                  <label>Dari</label>
                  <div class="input-group date">
  ...
  <script type="text/javascript">
    $(".btnedit").click(function(event) {
        ...
        $.getJSON('<?php echo base_url()."d.php/tkekurangan/tampil/" ?>'+record.find('#kode').html(), function(data) {
        $("#namadosen").val(data.id_dosen).trigger('change');
        $("#idthnajaran").val(data.thn_ajaran).trigger('change');
        $("#banyak").val(data.banyak).trigger('change');
        $("#bnominal").val(data.nominal_satuan);
        $("#datepicker3").val(data.periode_dari);
        $("#datepicker4").val(data.periode_sampai);
        $("#editform").attr("action", "<?php echo base_url()."d.php/tkekurangan/ubah/" ?>"+record.find('#kode').html());
    });
