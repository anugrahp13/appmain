<!-- =============================================== -->
  ...
  <section class="content-header">
    <div class="callout callout-info">
      ...
      Priode tambah transport di sesuaikan dengan periode cetak honor dosen.<br>
      <label>Informasi!</label>
      Banyak Transport di isi dengan angka bukan nominal.
    </div>
  </section>
  <!-- Finish Content Header -->
  ...
  <!-- Finihs Content Header -->
  <?php } ?>
  <!-- Main Content -->
  <section class="content">
    <?php if($this->session->flashdata('messageactive') == "ttransport"){?>
    ...
    <!-- Default Box -->
    <div class="box">
      <form action="<?php echo base_url();?>d.php/ttransport/hapus" method="post" id="myform">
        <!-- Box Body -->
        <div class="box-body">
          <!-- Table Bagian Atas -->
          <table id="example1" class="table table-bordered">
            <thead>
              <tr>
                ...
                <th>Nama Dosen</th>
                <th>Mata Kuliah</th>
                <th class="text-center">Banyak Transport</th>
                <th>Priode</th>
                <?php if($this->session->userdata('id_divisi') == "1"){ ?>
            ...
            <tbody>
              <?php
                $no = 0;
                foreach ($loadtransport as $key) {
                  $no++;
                  $a = $this->global_model->find_by('dosen_profil', array('id_dosen'=> $key['id_dosen']));
                  if($a != null){
              ?>
              <tr class="record">
                <td id="kode" style="display:none;"><?php echo $key['id_transport'];?></td>
                <td><input type="checkbox" name="check[]" value="<?php echo $key['id_transport'];?>"></td>
                <td><?php echo $no;?></td>
                <td>
                  <?php
                    $a = $this->global_model->find_by('dosen_profil', array('id_dosen'=>$key['id_dosen']));
                ...
                <td>
                  <?php
                    ...
                    if($c != ""){echo $c['nama_matkul'];}else{echo "-";}
                  ?>
                </td>
                <td class="text-center"><?php echo $key['banyak_transport'] ?></td>
                <td>
                  <?php
        ...
        <form method="post" action="" role="form">
          <div class="modal-body">
            <div class="row">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  <label>Banyak Transport</label>
                  <input type="number" name="banyak_transport" class="form-control" onkeypress="return isNumberKey(event)" required>
                </div>
                <div class="form-group">
                  <label>Dari</label>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span></button>
          <h4 class="modal-title">Hapus Data Transport</h4>
        </div>
        <div class="modal-body">
          ...
          <div class="modal-body">
            <div class="row">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  <label>Banyak Transport</label>
                  <input type="number" name="banyak_transport" class="form-control" onkeypress="return isNumberKey(event)" required id="btransport">
                </div>
                <div class="form-group">
                  <label>Dari</label>
                  <div class="input-group date">
  ...
  <script type="text/javascript">
    $(".btnedit").click(function(event) {
        ...
        $.getJSON('<?php echo base_url()."d.php/ttransport/tampil/" ?>'+record.find('#kode').html(), function(data) {
        ...
        $("#btransport").val(data.banyak_transport);
        $("#datepicker3").val(data.periode_dari);
        $("#datepicker4").val(data.periode_sampai);
        $("#editform").attr("action", "<?php echo base_url()."d.php/ttransport/ubah/" ?>"+record.find('#kode').html());
    });
